 ...
 * external functions
 * */
extern void flush_RX_FIFO(USB_t* usb);
extern void flush_TX_FIFO(USB_t* usb, uint8_t ep);
extern void flush_TX_FIFOS(USB_t* usb);


/*!<
 * IO
 * */
static inline void USB_write_packet(const USB_t *usb, uint8_t *src, uint8_t ep_num, uint16_t len) {
...
void IN_transfer(USB_handle_t* handle, uint8_t ep_num, void* buffer, uint32_t size) {
	ep_num &= 0xFU;
	USB_t* 			usb =		handle->instance;
	USB_device_t*	device =	(void*)(((uint32_t)usb) + USB_DEV_REL_BASE);
	USB_IEP_t*		in =		(void*)(((uint32_t)usb) + USB_IEP_REL_BASE + (ep_num * USB_EP_SIZE));
...
void OUT_transfer(USB_handle_t* handle, uint8_t ep_num, void* buffer, uint32_t size) {
	...
	USB_t* 			usb =		handle->instance;
	USB_device_t*	device =	(void*)(((uint32_t)usb) + USB_DEV_REL_BASE);
	USB_OEP_t*		out =		(void*)(((uint32_t)usb) + USB_OEP_REL_BASE + (ep_num * USB_EP_SIZE));
...
static inline void USB_OTG_IRQ(USB_handle_t* handle, USB_t* usb) {
...
static inline void USB_SOF_IRQ(USB_handle_t* handle, USB_t* usb ) {
	// TODO: redo handle and make it possible to pass usb ptr
	...
	if ((device->DSTS & USB_OTG_DSTS_SUSPSTS) == USB_OTG_DSTS_SUSPSTS) {
		...
		*PCGCCTL |= USB_OTG_PCGCCTL_STOPCLK;
		if (handle->config.low_power_enable) {
			SCB->SCR |= (uint32_t)((uint32_t)(0x6UL));
		}
	}
...
static inline void USB_enumeration_done_IRQ(USB_handle_t* handle) {
	...
	if		(PLLQ_clock_frequency < 15000000UL)	{ usb->GUSBCFG |= 0xFU << USB_OTG_GUSBCFG_TRDT_Pos; }
	else if	(PLLQ_clock_frequency < 16000000UL)	{ usb->GUSBCFG |= 0xEU << USB_OTG_GUSBCFG_TRDT_Pos; }
	else if	(PLLQ_clock_frequency < 17200000UL)	{ usb->GUSBCFG |= 0xDU << USB_OTG_GUSBCFG_TRDT_Pos; }
	else if	(PLLQ_clock_frequency < 18500000UL)	{ usb->GUSBCFG |= 0xCU << USB_OTG_GUSBCFG_TRDT_Pos; }
	else if	(PLLQ_clock_frequency < 20000000UL)	{ usb->GUSBCFG |= 0xBU << USB_OTG_GUSBCFG_TRDT_Pos; }
	else if	(PLLQ_clock_frequency < 21800000UL)	{ usb->GUSBCFG |= 0xAU << USB_OTG_GUSBCFG_TRDT_Pos; }
	else if	(PLLQ_clock_frequency < 24000000UL)	{ usb->GUSBCFG |= 0x9U << USB_OTG_GUSBCFG_TRDT_Pos; }
	else if	(PLLQ_clock_frequency < 27700000UL)	{ usb->GUSBCFG |= 0x8U << USB_OTG_GUSBCFG_TRDT_Pos; }
	else if	(PLLQ_clock_frequency < 32000000UL)	{ usb->GUSBCFG |= 0x7U << USB_OTG_GUSBCFG_TRDT_Pos; }
...
static inline void USB_wakeup_handler(USB_handle_t* handle) {
	...
	*PCGCCTL &= ~(USB_OTG_PCGCCTL_STOPCLK);
	EXTI->PR |= (0b1UL << 18U);  // USB FS EXTI line TODO: modular
}
